...
(async function () {
    ...
    function renderMonth(date, eventsMap, holidaysMap) {
        ...
        for (let d = 1; d <= daysInMonth; d++) {
            ...
            // Contadores
            if (evs.length > 0) {
                const working = evs.filter(e => {
                    const shift = (e.shift || '').toUpperCase();
                    return shift.includes('AGENDA') || shift.includes('FOLGA VENDIDA');
                }).length;
    ...
    function formatShiftLabel(shift) {
        ...
        if (normalized === 'AGENDA + FOLGA VENDIDA') return 'Agenda + Folga vendida';
    ...
    function openDayPanel(iso, day, month, year, eventsMap, holidaysMap) {
        ...
        evs.forEach(ev => {
            ...
            if (s.includes('AGENDA') || s.includes('FOLGA VENDIDA'))                   groups['AGENDA'].push(ev);
